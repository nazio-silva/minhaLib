...
public class BulbCharacteristicHandler {
    ...
    public HashMap<OrderType, BulbCharacteristic> getCharacteristics(final BluetoothGatt gatt) {
            ...
            if (service.getUuid().toString().startsWith(SERVICE_UUID_HEADER_NOTIFY)) {
                for (BluetoothGattCharacteristic characteristic : characteristics) {
                    ...
                    if (characteristicUuid.equals(OrderType.axisData.getUuid())) {
                        gatt.setCharacteristicNotification(characteristic, true);
                        bulbCharacteristicMap.put(OrderType.axisData, new BulbCharacteristic(characteristic, OrderType.axisData));
                        continue;
                    }
                    if (characteristicUuid.equals(OrderType.htData.getUuid())) {
                        gatt.setCharacteristicNotification(characteristic, true);
                        bulbCharacteristicMap.put(OrderType.htData, new BulbCharacteristic(characteristic, OrderType.htData));
                        continue;
                    }
                    if (characteristicUuid.equals(OrderType.htSavedData.getUuid())) {
                        gatt.setCharacteristicNotification(characteristic, true);
                        bulbCharacteristicMap.put(OrderType.htSavedData, new BulbCharacteristic(characteristic, OrderType.htSavedData));
                        continue;
                    }
                    ...
                    if (characteristicUuid.equals(OrderType.deviceType.getUuid())) {
                        bulbCharacteristicMap.put(OrderType.deviceType, new BulbCharacteristic(characteristic, OrderType.deviceType));
                        continue;
                    }
                    if (characteristicUuid.equals(OrderType.slotType.getUuid())) {
                        bulbCharacteristicMap.put(OrderType.slotType, new BulbCharacteristic(characteristic, OrderType.slotType));
                        continue;
                    }
            ...
            if (service.getUuid().toString().startsWith(SERVICE_UUID_HEADER_EDDYSTONE)) {
                for (BluetoothGattCharacteristic characteristic : characteristics) {
                    ...
                    if (characteristicUuid.equals(OrderType.connectable.getUuid())) {
                        bulbCharacteristicMap.put(OrderType.connectable, new BulbCharacteristic(characteristic, OrderType.resetDevice));
                        continue;
                    }
                }
            }
